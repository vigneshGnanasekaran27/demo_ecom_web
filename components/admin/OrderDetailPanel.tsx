...
export function OrderDetailPanel({ orderId }: { orderId: number }) {
  ...
  if (isPending) {
    return <div className="h-32 animate-pulse rounded-lg bg-zinc-100" aria-busy="true" />;
  }

  if (isError || !order) {
    return <p className="text-sm text-red-600">Unable to load order details.</p>;
  }
  ...
  return (
    <div className="grid gap-6 text-sm md:grid-cols-3">
      <div>
        ...
        <h4 className="mb-2 mt-4 text-xs font-semibold uppercase tracking-wide text-zinc-500">
          Shipping Address
        </h4>
        <p className="text-zinc-600">
        ...
        {order.shipping.landmark && <p className="text-zinc-600">Landmark: {order.shipping.landmark}</p>}
      </div>
      ...
      <div>
        ...
        {order.payments.length === 0 ? (
          <p className="text-zinc-500">No payment recorded yet.</p>
        ) : (
          order.payments.map((payment) => (
            <div key={payment.id} className="mb-2 rounded-md border border-zinc-200 p-2">
              <p className="font-medium capitalize text-zinc-900">{payment.status}</p>
              <p className="text-zinc-600">{formatPriceCents(payment.amount_cents)}</p>
        ...
        <p className="text-zinc-600">Status: {ORDER_STATUS_LABELS[order.order_status]}</p>
        <p className="text-zinc-600">Placed: {order.placed_at ? dateFormatter.format(new Date(order.placed_at)) : "—"}</p>
        <Link
        ...
        <ul className="space-y-1">
          {order.items.map((item) => (
            <li key={item.id} className="flex justify-between text-zinc-600">
              ...
              <span className="flex-shrink-0 text-zinc-900">{formatPriceCents(item.line_total_cents)}</span>
            </li>
          ))}
        </ul>
