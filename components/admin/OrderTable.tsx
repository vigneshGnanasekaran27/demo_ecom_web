...
function statusBadge(order: AdminOrderListItem): { label: string; className: string } {
  ...
  if (["confirmed"].includes(order.order_status)) {
    return { label: ORDER_STATUS_LABELS[order.order_status], className: "bg-blue-100 text-blue-700" };
  }
  return {
...
function OrderRow({
  ...
  return (
    <>
      <tr className="border-b border-zinc-100 transition-colors last:border-b-0 hover:bg-zinc-50/70">
        ...
        <td className="px-5 py-3.5 font-medium text-zinc-900">{order.order_number}</td>
        <td className="px-5 py-3.5 text-zinc-600">{order.customer_name}</td>
        <td className="px-5 py-3.5 text-zinc-600">{dateFormatter.format(new Date(order.created_at))}</td>
        <td className="px-5 py-3.5 text-zinc-900">{formatPriceCents(order.total_cents)}</td>
        <td className="px-5 py-3.5">
          <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${badge.className}`}>{badge.label}</span>
        </td>
        <td className="px-5 py-3.5">
          {nextOptions.length === 0 ? (
            <span className="text-xs text-zinc-400">No further action</span>
          ) : (
